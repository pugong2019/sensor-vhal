...
sock_client_t *sock_client_init(int type, const char *server_path, int port) {
  ...
  UNUSED(path);
  SOCK_CLIENT_LOG(
      ("sock_client_init(%s, %s, %d)  ...", type == SOCK_CONN_TYPE_INET_SOCK ? "INET" : "UNIX", server_path, port));
  ...
  SOCK_CLIENT_LOG(("sock_client_init(%s, %s, %d) returns %p", type == SOCK_CONN_TYPE_INET_SOCK ? "INET" : "UNIX",
                   server_path, port, client));
...
int sock_client_send_fd(sock_client_t *client, int *pfd, size_t fdlen) {
  ...
  do {
    ...
    if (count < 0) {
#if DEBUG_SOCK_CLIENT
      sock_log("%s : %d : sendmsg failed, count = %d, error = %d (%s)", __func__, __LINE__, count, errno,
               strerror(errno));
#endif
    }
...
int sock_client_recv_fd(sock_client_t *client, int *pfd, size_t fdlen) {
  ...
  do {
    ...
    if (count < 0) {
#if DEBUG_SOCK_CLIENT
      sock_log("%s : %d : recvmsg failed, count = %d, error = %d (%s)", __func__, __LINE__, count, errno,
               strerror(errno));
#endif
    }
  } while ((count < 0) && ((errno == EINTR) || (errno == EAGAIN)));
